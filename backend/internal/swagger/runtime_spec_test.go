...
func TestRuntimeSpecsAreValidAndCoverRegisteredAPIRoutes(t *testing.T) {
	tests := []struct {
		...
		{name: "user", raw: UserSpec, methodCount: 28},
		{name: "staff", raw: StaffSpec, methodCount: 60},
	}
...
func TestAIChatSwaggerIsStaffOnlyAndUsesIntegerBusinessIDs(t *testing.T) {
	var userDoc map[string]any
	if err := json.Unmarshal(UserSpec, &userDoc); err != nil {
		t.Fatal(err)
	}
	if _, ok := userDoc["paths"].(map[string]any)["/api/ai/chat"]; ok {
		t.Fatal("user API must not expose AI chat")
	}
