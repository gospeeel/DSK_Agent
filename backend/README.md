...
3. **Go User Server (`:8080`)** — клиентский HTTP API:
   ...
   - Чат с менеджерами и AI-консультантом.
...
- **Сценарий 5 (Общий вопрос к ИИ и эскалация на оператора):** Клиент задает общий вопрос в `POST /api/ai/chat`, сообщение уходит в Agent Service по RabbitMQ и сохраняется в диалоге; при необходимости позвать человека обращение эскалируется в открытую очередь для менеджеров.
...
Эндпоинт доступен на обоих серверах:
- `POST /api/ai/chat`
...
```
*Примечание: Если пользователь авторизован, backend автоматически передает его ID из токена в RPC-сообщение; если передан `session_id`, вопрос и ответ сохраняются в историю сообщений чата.*
